...
ADD:	add $v0,$v0,$s6 					# neu la chuc nang 2 thi ADD $v0 = $v0 + $s6 
END_ADD:
li $t1,2
...
chuc_nang_3:	bne $s6,$0,SWAP 				# neu $s6 = 5 thi thuc hien dao ve vi tri ban dau khoi tao
		j end_xu_ly_dao 				# thuc hien nhap
xu_ly_dao: 	li $s7,10
		j SWAP 						# sau khi dao ve vi tri ban dau thuc hien swap 
end_xu_ly_dao:	la $a0,nhap1 					# print "mau chu D"
...
chuc_nang_4: 	bne $s6, $0,SWAP
		j reset
chuc_nang_5:  	li $v0,10
		syscall
reset:		li $s6,0
		li $s3,50 					# mau D = 2
		li $s4,49 					# mau C = 1
...
nhap_lai:	li $v0,4
		...
		add $a0,$a1,$0
		j nhap_mau
MAU1:	add $v1, $s3,$0 					# $v1 la bien thay doi mau 
...
duyet:	add $t1,$k0,$t0 					# address i # duyet tung ki tu trong khoang $a2 - $a3
	lb $t2,0($t1) 						# value i
...
printf: # thuc hien in tung dong
	...
	li $t1,0
	j loop
DAO:	li $t0,0 						# khoi tao i = 0
loop2:	add $t1,$a0,$t0 					# duyet tung phan tu
	...
	addi $t0,$t0,1 						# i = i + 1
	j loop2
end_loop2: jr $ra 						# quay lai duyet dong tiep theo
END_DAO: 
XU_LY_VIEN: 	add $t1,$a1,$t0 				# thuc hien duyet tung ki tu - $a1 la bien luu tru line i
		lb $a0,0($t1)
		li $t1,10 					# t1 = '\n'
...
print_32:	li $v0,11 					# print " "
		li $a0,32
		syscall
jump_xu_ly_vien:addi $t0,$t0,1 					# quay lai vong lap
		j XU_LY_VIEN
end_xu_ly_vien: li $v0,11					# print "\n"
		syscall
		li $t0,0 					# khoi tao lai i = 0
		jr $ra 						# quay lai duyet dong tiep theo
khong_co_chuc_nang: 	li $v0,4 				# thong bao loi 
			la $a0,thong_bao
			syscall
j loop
end:

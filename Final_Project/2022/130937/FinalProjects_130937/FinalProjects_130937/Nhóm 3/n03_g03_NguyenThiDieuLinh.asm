...
.data 
            bytehex     : .byte 0x3F,0x6,0x5B,0x4F,0x66,0x6D,0x7D,0x7,0x7F,0x6F 
...
SHOW_KEY:    sb $t0, 0($s0)                 #hien thi ky tu vua nhap tu ban phim tren man hinh MMIO
...
#ma ngat xay ra, tuc là lenh dã thuc hien xong, chu không chua dia chi cua 
#lenh ke tiep. Boi vây phi tu lap trình de tang dia chi chua trong thanh ghi 
#epc bang cách su dung 2 lenh mfc0 (de doc thanh ghi trong bo dong xu lý 
#C0) và mtc0 (de ghi gia tri vao thanh ghi trong bo dong xu ly C0) 
#---------------------------------------------------                                   
...
PRINT:      
	...
	mtc1 $s4, $f1		# Convert $s4(integer) -> $f1(float): so luong phim da nhap
	mtc1 $s5, $f2		# Convert $s5(integer) -> $f2(float): so luong time (so chu ky)
